...
public class ScatterCommand extends STMCommand {
    ...
    public static void execute(CommandSender caller, String[] args) {        
        if (!caller.isOp() && !caller.hasPermission("stm.scatter")) {
            STMCommand.notifyUsage(caller, WrongUsageType.NO_PERM, CommandUsage.SCATTER);
            return;
        }
        
        if (!(caller instanceof Player)) {
            caller.sendMessage(ChatColor.RED + "Need to be player to execute this command");
        }
        if (args.length < 3) {
            STMCommand.notifyUsage(caller, WrongUsageType.ARGS_FEW, CommandUsage.SCATTER);
            return;
        }
        if (!args[2].contains(",")) {
            STMCommand.notifyUsage(caller, WrongUsageType.WRONG_ARGS, CommandUsage.SCATTER);
        }
        ...
        int radius;
        try {
            centre = new int[] { Integer.parseInt(centreString[0]), Integer.parseInt(centreString[1]) };
            radius = Integer.parseInt(args[1]);
        }
        catch (NumberFormatException e) {
            notifyUsage(caller, WrongUsageType.WRONG_ARGS, CommandUsage.SCATTER);
            return;
        }
        
        Player p = (Player) caller;
        TeamHandler handler = SystemTeamManager.getTeamHandler();
        Location centreLoc = new Location(p.getWorld(), centre[0], 0.0D, centre[1]);
        ArrayList<Location> locations = TeamScatter.getScatterLocations(radius, handler.getTeams().size(), centreLoc);
